...
const InputBar = (props) => {
  ...
  <>
        ...
        <div class="md:container  mx-auto mt-32 bg-[#e8e8e8] rounded-lg h-50 p-20">
        {loading? 
        
        <Spinner/>
         : 
         <>
          <div className="bg-[#005471] w-full">
             <h1 className="text-white">Please Enter Your {service.input} Number</h1>
          </div>
          <form  className="p-5" onSubmit={handleGoState}>
                ...
                <div class="flex items-center justify-center p-3 mb-5">
                    ...
                    <h6 className="font-normal text-black lg:text-2xl md:text-xs mr-4">{service.input == "RID"? <FormattedMessage id={"enter-rid"}/>: <FormattedMessage id={"enter-uin"}/>}</h6> 
                   <input class=" block md:w-full lg:w-1/3 p-4  md:placeholder:text-left text-sm text-gray-900 border border-gray-300 rounded-lg bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value={id} placeholder={service.input}  onChange={handleChange}
                  maxLength="16" minLength="16" required/>
                </div>
                <div className="flex items-center justify-center p-1 mr-56 md:p-2">
                  <h6 className="font-normal text-black  lg:text-2xl md:text-xs mr-4"><FormattedMessage id={"select-OTP-method"}/>: </h6>
                  <DropdownSelector options={['send-phone', 'send-email']} onCallback = {handleCallback}/>
                </div> 
       
                <div class="justify-center items-center p-1 mr-96">
                    <button type="submit"  class="inline-block px-7 py-3  bg-[#005471] text-white font-small text-base w-1/4 leading-tight uppercase rounded-xl shadow-md hover:bg-[#083247] hover:shadow-lg focus:bg-[#214b60] focus:shadow-lg focus:outline-none focus:ring-0 active:bg-[#304f55] active:shadow-lg transition duration-150 ease-in-out"><FormattedMessage id="go"/></button>
                </div>
